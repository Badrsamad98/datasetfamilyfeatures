...
def draw_bbox_landm(img, ann, img_height, img_width):
    ...
    if ann[14] > 0:
        cv2.circle(img, (int(ann[4] * img_width),
                         int(ann[5] * img_height)), 1, (255, 255, 0), 3*int(img_height/800))
        cv2.circle(img, (int(ann[6] * img_width),
                         int(ann[7] * img_height)), 1, (0, 255, 255), 3*int(img_height/800))
        cv2.circle(img, (int(ann[8] * img_width),
                         int(ann[9] * img_height)), 1, (255, 0, 0), 3*int(img_height/800))
        cv2.circle(img, (int(ann[10] * img_width),
                         int(ann[11] * img_height)), 1, (0, 100, 255), 3*int(img_height/800))
        cv2.circle(img, (int(ann[12] * img_width),
                         int(ann[13] * img_height)), 1, (0, 0, 255), 3*int(img_height/800))
    marks = []
    for i in range(4, 13, 2):
        marks.append([int(ann[i] * img_width), int(ann[i+1] * img_height)])
    M = np.asarray(marks)
#    p = r'F:\M.tech Projects\Offspring recog\retinaface-tf2-master\retinaface-tf2-master\data\marks.txt'
    return M
